...
<<read-coverage, fig.pos="ht", fig.width=5, fig.asp=1/2, fig.align='center', fig.cap="Hello">>=
ggplot(counts_histogram, aes(x=counts)) + geom_histogram(color="black", fill="white", bins=20) + theme_classic()
@

Library complexity refers to the number of unique DNA fragments present in a given library. To compute library complexity, we used a sampling-based approach. For each patient, we sampled the genes with probability proportional to the read count of each gene for that patient. In figure~\ref{fig:library-complexity} we plot the library complexities for all samples.
Analysing the figure we can see that, in general, samples from normal tissues has a higher??Lower?? library complexity than samples from diseased tissues, i.e.
<<library-complexity, fig.pos="ht", fig.asp=2/3, fig.align='center', fig.cap="Hello", cache=TRUE>>=
groups = as.numeric(substr(colnames(read_counts), nchar(colnames(read_counts))-1, nchar(colnames(read_counts))-1))
