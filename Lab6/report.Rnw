...
Next, we repeated te proccess, but this time the differential expression of each gene was computed with respect not only to whether the sample is healthy or has a tumour but also to the age of the patient. The results are shown in table~\ref{tab:2d2}.
...
The ROC curve for read counts of ESR1 gene is shown in figure~\ref{fig:estrogen2}. We can see that it is possible to get a TPR \(\approx 90\%\) and a TNR \(\approx 10\%\) using this classifier, which adds up to a very good classification accuracy.
...
<<survival, fig.pos="ht", fig.asp=3/4, fig.align='center', fig.cap="Hello">>=
