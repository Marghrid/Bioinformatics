...
We can see that the top 5 genes are exactly the same, from which we conclude that the age of the patient does not affect the differences in expressed genes between tumourtumours and normal breast samples.

<<2d, fig.pos="ht", fig.width=5, fig.height=4, fig.align='center', fig.cap="Vooms?", cache=TRUE>>=
dge <- DGEList(counts=data.matrix(read_counts))
keep <- filterByExpr(dge)
dge <- dge[keep, , keep.lib.sizes=FALSE]
dge <- calcNormFactors(dge)
...
\todo{explain confusion}

<<knntop5-conf, fig.pos="ht", fig.width=5, fig.height=4, fig.align='center', fig.cap="Confusion matrix of leave-one-out cross-validation 5-Nearest Neighbour classifier", cache=TRUE, message=FALSE>>=
...
Despite all our efforts, the results are disheartening (and shown in figure~\ref{fig:knnpam50-conf}).

\todo{explain and compare confusion}

<<knnpam50-conf, fig.pos="ht", fig.width=5, fig.height=4, fig.align='center', fig.cap="a", cache=TRUE, message=FALSE>>=
...
We used our KNN classifier from exercise~\ref{sec:3c}. To visualise the results we produced the MDS plot that can be seen in figure~\ref{fig:classifier-2}
