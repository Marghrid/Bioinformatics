...
tumours and normal breast samples?}
...
eBfit = eBayes(linearfit)
volcanoplot(eBfit,coef=2,style="B-statistic")
kable(topTable(eBfit,coef=2,number=5), format = "latex", booktabs = TRUE, caption = "Top 5 most differentially expressed genes considering both the tumour classification of the tissue and the age of the patient", table.envir = "table*", lable="2d2")
@
...
We compute once again the differencial gene expression, as in exercise \ref{sec:2d}, only this time it is with respect to the PAM50 classidication. We pick the top 5 most differentially expressed genes, which are shown in table~\ref{tab:dif-expr-pam50}. \todo{How many of the genes in your signature are
part of the PAM50 gene list? See on PDF}
...
To test this classifier, we performed leave-one-out cross-validation: from our N classified samples, we used N\(-1\) as training data, and the remaining one as test data. We show the results of this in the form of a confusion matrix, in figure~\ref{fig:knn-conf}.

<<knn-pam50, fig.pos="ht", fig.width=5, fig.height=4, fig.align='center', fig.cap="a", cache=TRUE, message=FALSE>>=
...
<<knn-conf, fig.pos="ht", fig.width=5, fig.height=4, fig.align='center', fig.cap="a", cache=TRUE, message=FALSE>>=
